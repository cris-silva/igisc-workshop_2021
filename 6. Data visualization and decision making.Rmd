...
```{r Create a hexgrid states layer}
...
states_hex <- 
  ...
  select(clave_ent, nom_abreviado, hom_10khab) %>% 
  glimpse()
```
...
states_offenses %>% 
  ...
  select(entidad, hom_10khab) %>% # Pick the variables to be showed
...
states_offenses %>% 
  ...
  arrange(ranking) %>% 
  select(ranking, entidad, homicidio, hom_10khab, participacion) %>% 
