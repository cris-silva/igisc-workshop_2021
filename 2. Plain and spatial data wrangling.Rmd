...
Load the `tidyverse` package that will be used to manipulate the data:
...
offenses <- 
  read_csv(file = data_source,
           locale = locale(encoding = "WINDOWS-1252")) %>%  # locale parameter required in macOS or Linux, data comes from Windows 
...
In order to filtering by date, it is necessary to have a `month` column in date format in a *longer* format. To achieve this, the next chunk will execute the next operations:

1. Transform structure to *longer* format, storing each month name (in spanish) from column title in a variable named `month_name` and its values in a variable named `total` using `pivot_longer` function:.
2. Get the month number from `month_name` using `case_when` function to store the corresponding numeric value it on a new variable named `month_number`.
3. Build the month in date format using `ano` (year), `month_number` and the number 1 to assume the first day using the `lubridate` function `make_date` to create a new variable called `month`.
4. Finally omit the unnecessary variables `month_name`, `month_number`, `ano` using the function `select` and the `-` preffix to exclude them in the dataset.
...
offenses <-
  ...
  pivot_longer(cols = 8:19,
               names_to = "month_name",
               values_to = "total") %>% 
  mutate(month_number = case_when(month_name == "enero" ~ 1,
  ...
  mutate(month = make_date(ano, month_number, 1)) %>% 
  select(-ano, -month_name, -month_number) %>% 
  glimpse()
```
...
Filter data to keep only with the data corresponding to `Homicidio` (murder) and its subcategories, that happened from december 2018 to june 2021:
```{r Subset data}
...
murders <-
  offenses %>% 
  filter(tipo_de_delito %in% c("Homicidio", "Feminicidio") & between(month, month_begin, month_end)) %>% 
  glimpse()
```

Group and summarize murder totals by state:
```{r Grouping and summarizing data}
murders_by_state <-
  murders %>% 
  group_by(clave_ent, entidad) %>% 
  summarize(murders = sum(total, na.rm = TRUE)) %>% 
  ungroup() %>% 
  glimpse()
```
...
By using `sf` package it is possible to read plain text data with geometry such as coordinates o well-known-text (WKT), and then convert it to *simple features* (spatial data) following the next instructions. Read the plain text data using `read_csv` function:
...
Filter murder data for the same period of time, in this dataset the column `fecha_hechos` (event date) will be used:
```{r Subset data for the city}
murders_cdmx <-
  offenses_cdmx %>% 
  mutate(month = date(fecha_hechos)) %>% # Create a date column for "month" from datetime column "fecha_hechos" in order to filter
  filter(categoria_delito == "HOMICIDIO DOLOSO" & between(month, month_begin, month_end)) %>% 
  glimpse()
```

This dataset uses a WGS84 geographic coordinate reference system (CRS). Use the coordinates contained in columns `longitud` (longitude) and `latitud` (latitude) to build point geometries and convert dataset to *simple features* using `st_as_sf` and `st_set_crs` functions:
