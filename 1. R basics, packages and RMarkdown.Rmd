...
Vectors are basic data structures used in R to compute multiple values at once. Create a vector using `c()`:
```{r Create vector}
...
Select a particular element in a vector:
```{r Select by index}
...
my_df_from_csv2 <- read.csv2("Data/my_df.csv")
...
```{r Remove a variable}
rm("my_number") # Remove a variable
